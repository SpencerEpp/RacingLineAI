...
def get_racing_line(images_dir, model_path):
    ...
    for ext in image_exts:
        images.extend(glob(os.path.join(images_dir, ext)))
    images = sorted(images)
    total_images = len(images)
    print(f"Found {total_images} layout files.\n")

    for image_index, image_path in enumerate(tqdm(images, desc="Predicting Layouts")):
        image_name = os.path.basename(image_path)
        
        tqdm.write(f"[{image_index + 1}/{total_images}] Predicting layout: {image_name}")

        track_data = create_inference_data(image_path)

        patches = t.tensor(np.array(track_data["patches"]), dtype=t.float32).unsqueeze(1).to(device)
        center = t.tensor(np.array(track_data["center"]), dtype=t.float32).to(device)

        with t.inference_mode():
            control_pred, position_pred = model(patches, center, meta=None)
        ...
        plot_colored_racing_line(track_data["track_image"], segments, speed_val, cmap="viridis",
                                label="Speed", title=f"Speed Overlay: {image_name}")
        plot_colored_racing_line(track_data["track_image"], segments, gas_val, cmap="Greens",
                                label="Gas", title=f"Gas Pedal Overlay: {image_name}")
        plot_colored_racing_line(track_data["track_image"], segments, brake_val, cmap="Reds",
                                label="Brake", title=f"Brake Pedal Overlay: {image_name}")